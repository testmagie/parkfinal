...
def book_slot():
    ...
    amount_paid = duration * rate_per_hour  # Calculate the amount based on rate per hour
...
def admin_login():
    if request.method == 'POST':
        ...
        if username == ADMIN_USERNAME and password == ADMIN_PASSWORD:
            return redirect(url_for('admin'))  # Redirect to admin page if credentials are correct
        else:
            # Show an error popup if login is incorrect
            return render_template('admin_login.html', error_message="Incorrect username or password!")
    return render_template('admin_login.html')

@app.route('/admin', methods=['GET','POST'])
def admin():
    # Assuming you have an actual route and logic for the admin page
    try:
        with sqlite3.connect('database.db') as conn:
            cursor = conn.cursor()
            cursor.execute(''' 
                SELECT slot_number, vehicle_type, vehicle_number, vehicle_owner, in_time, out_time, 
                       payment_status, amount_paid, is_occupied, penalty_amount
                FROM parking_slots
            ''')
            slots = cursor.fetchall()
