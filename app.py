...
def book_slot():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        if result:
            ...
            cursor.execute('''SELECT slot_number FROM parking_slots WHERE vehicle_type = (SELECT vehicle_type FROM parking_slots WHERE slot_number = ?) AND is_occupied = 0''', (existing_slot_number,))
            available_slots = cursor.fetchall()

            message = "This vehicle number is already booked."
            if existing_slot_number <= 10:  # Car slots
                return render_template('book_car.html', message=message, slots=available_slots, vehicle_number=vehicle_number, vehicle_owner=vehicle_owner, in_time=in_time, out_time=out_time, rate_per_hour=5)
            else:  # Bike slots
                return render_template('book_bike.html', message=message, slots=available_slots, vehicle_number=vehicle_number, vehicle_owner=vehicle_owner, in_time=in_time, out_time=out_time, rate_per_hour=2)
